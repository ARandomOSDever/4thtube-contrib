...
u32 draw_japanese_kanji[3001];
int japanese_kanji_num;
u32 draw_simple_chinese[6301];
		...
		for (int k = 0; k < y_max; k++) {
			for (int i = 0; i < x_max; i += 2) {
				memcpy_asm_4b(&(((u8 *)c2d_image->c2d.tex->data)[c3d_pos + c3d_offset]),
				              &(((u8 *)buf)[Draw_convert_to_pos(k + parse_start_height, i + parse_start_width,
				                                                pic_height, pic_width, pixel_size)]));
				memcpy(&(((u8 *)c2d_image->c2d.tex->data)[c3d_pos + c3d_offset + 4]),
				       &(((u8 *)buf)[Draw_convert_to_pos(k + parse_start_height, i + parse_start_width, pic_height,
...
void Draw(std::string text, float x, float y, float text_size_x, float text_size_y, int abgr8888) {
	...
	bool found = false;
	bool font_loaded[2] = {
	    Extfont_is_sysfont_loaded(0),
	    Extfont_is_sysfont_loaded(1),
	}; // JPN, CHN
	float width = 0, height = 0, original_x, y_offset;
	static int font_num_list[1024];
	static u32 draw_part_text[1024];
		...
		if (prev_font_list_num != cur_font_list_num || cur_font_list_num == -1 ||
			...
			if (prev_font_list_num == -1) { // linebreak
				...
				for (int j = consecutive_start; j < i; j++) {
					...
					if (cur_char >> 16) {
						draw_str.push_back(cur_char >> 16 & 0xFF);
					}
					if (cur_char >> 8) {
						draw_str.push_back(cur_char >> 8 & 0xFF);
					}
...
void Draw_debug_info(u8 val) {
	...
	switch (val) {
	case 1:
		Draw_texture(var_square_image[0], DEF_DRAW_WEAK_BLUE, 0.0, 18.0, 70.0, 220.0);
		Draw_texture(var_square_image[0], DEF_DRAW_WEAK_BLUE, 70.0, 158.0, 40.0, 80.0);
...
Result_with_string Draw_load_kanji_samples(void) {
	...
	if (result.code == 0) {
		japanese_kanji_num = Extfont_parse_utf8_str_to_u32((const char *)fs_buffer, draw_japanese_kanji, 3000);
	}

	memset((void *)fs_buffer, 0x0, 0x8000);
	result = Path("romfs:/gfx/font/sample/hanyu_s.txt").read_file(fs_buffer, 0x8000, size_read);
	if (result.code == 0) {
		simple_chinese_num = Extfont_parse_utf8_str_to_u32((const char *)fs_buffer, draw_simple_chinese, 6300);
	}
...
Result_with_string Draw_init(bool wide) {
	...
	for (int i = 0; i < 8; i++) {
		samples[i] = str2u32(samples_str[i]);
	}
